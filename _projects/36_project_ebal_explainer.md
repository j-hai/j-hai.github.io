...
The Stata package's `targets(1)` option matches first moments only;
`targets(2)` adds variances; `targets(3)` adds co-moments. The
resulting weights variable plugs straight into any downstream Stata
estimator (`regress [pw = w]`, `teffects ipw`, `svyset`, etc.).
